...
router.route("/:action/:id").put(async (req, res) => {
  ...
  if ((selectedRequest) ) {
    if (actionAvailable) {
      ...
      res.status(404).json({success: false, allRequests, msg: "Action not available, request has already accepted or declined"})
    } 
  }
  else {
    res.status(404).json({success: false, allRequests, msg: "Request not available"})
  }
});
